...
# export windyStore to csv
import csv
with open('windyStore.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(windyStore)
...
ax1[0].axhline(y=targetMAP, color ="red", label = 'Target = {}'.format(targetMAP))
#ax1[0].axhline(y=0, color ="red")
ax1[0].axhline(y=restingHighBP, color = "purple", label = 'Resting High BP = {}'.format(restingHighBP))
ax1[0].axhline(y=((restingHighBP+targetMAP)/2), color = "pink" )
ax1[0].plot(xBP, bloodPressureArray, color = "green", linewidth = 1.5, label = 'BP          | Initial = {}'.format(originalBP))
