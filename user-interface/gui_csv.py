...
UIFontDictionary.preload_fonts({'name': 'roboto', 'point_size': 15, 'style': 'regular', 'antialiased': '1'})
UIFontDictionary.preload_fonts({'name': 'roboto', 'point_size': 20, 'style': 'regular', 'antialiased': '1'})
...
filename = "./DashExportData.csv"
csv_params = []
csv_rows = []
system_has_data = False
...
while not done:
    ...
    with open(filename, 'r') as csvfile:
        ...
        csv_params = next(csvreader)
    
        # extracting each data row one by one
        for row in csvreader:
            csv_rows.append(row)
    # last_row = csv_rows[len(csv_rows) - 1]
    last_row = csv_rows[235]
    # print("time: ", last_row[0], " spO2: ", last_row[1])
    if last_row[1].isnumeric() and not system_has_data:
        print("data read! ", last_row[1])
        system_has_data = True
        current_map = int(last_row[1])
        initial_map = int(last_row[1])
        bp_log = []
    ...
    if not system_has_data:
        print("no data yet")
    
    # PID controller (run every timestep)
    if counter % fps == 0 and system_has_data:
